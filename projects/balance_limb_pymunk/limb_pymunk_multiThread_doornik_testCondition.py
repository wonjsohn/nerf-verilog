...
#        self.gForearm_shape.friction = 10.0
    ...
    """ set MN Ia gain """
    def setMNIaGain(self):
        bitVal = convertType(self.MNIaGain, fromType = 'f', toType = 'I')
        xem_muscle_bic.SendPara(bitVal = bitVal, trigEvent = 3) 
        xem_muscle_tri.SendPara(bitVal = bitVal, trigEvent = 3) 
    
    """ set MN II gain """
    def setMNIIGain(self):
        bitVal = convertType(self.MNIIGain, fromType = 'f', toType = 'I')
        xem_muscle_bic.SendPara(bitVal = bitVal, trigEvent = 11) 
        xem_muscle_tri.SendPara(bitVal = bitVal, trigEvent = 11) 
        
    """ set MN-CN gain """
    def setMNCNGain(self):
        bitVal = convertType(self.MNCNGain, fromType = 'f', toType = 'I')
        xem_muscle_bic.SendPara(bitVal = bitVal, trigEvent = 10) 
        xem_muscle_tri.SendPara(bitVal = bitVal, trigEvent = 10) 
        

        
    """ set SN Ia gain """
    def setSNIaGain(self):
        bitVal = convertType(self.SNIaGain, fromType = 'f', toType = 'I')
        xem_spindle_bic.SendPara(bitVal = bitVal, trigEvent = 1) 
        xem_spindle_tri.SendPara(bitVal = bitVal, trigEvent = 1) 
    
    """ set SN II gain """
    def setSNIIGain(self):
        bitVal = convertType(self.SNIIGain, fromType = 'f', toType = 'I')
        xem_spindle_bic.SendPara(bitVal = bitVal, trigEvent = 10) 
        xem_spindle_tri.SendPara(bitVal = bitVal, trigEvent = 10) 
        
    
    """ set CN Ia gain """
    def setCNIaGain(self):
        bitVal = convertType(self.CNIaGain, fromType = 'f', toType = 'I')
        xem_cortical_bic.SendPara(bitVal = bitVal, trigEvent = 1) 
        xem_cortical_tri.SendPara(bitVal = bitVal, trigEvent = 1) 
    
    """ set CN II gain """
    def setCNIIGain(self):
        bitVal = convertType(self.CNIIGain, fromType = 'f', toType = 'I')
        xem_cortical_bic.SendPara(bitVal = bitVal, trigEvent = 2) 
        xem_cortical_tri.SendPara(bitVal = bitVal, trigEvent = 2) 
...
#       bitVal = convertType(val, 'I', 'f')
    ...
    def controlLoopBiceps(self):
        ...
        while self.running:
            ...
            force_bic_pre = force_bic_pre*self.torqueMultiplier
...
#            self.linearV = 0.0
    #        print linearV
            self.scale = 10.0 #150.0   # unstable when extra cortical signal is given
...
#            xem_spindle_tri.SendPara(bitVal = bitVal, trigEvent = 9)
    ...
    #        print "force0 = %.2f :: force1 = %.2f :: angle = %.2f :: gd_bic = %.2f :: angularV =%.2f" % (force_bic, force_tri,  angle,  gd_bic,  angularV )                          
            ...
            self.elapsedTime = currentTime- self.start_time
            tempData = self.elapsedTime,  lce_bic, self.linearV, spikecnt_bic, force_bic, emg_bic#,  MN1_spikes,  MN2_spikes, MN3_spikes,  MN4_spikes,  MN5_spikes, MN6_spikes  
            if (self.record):
...
#            time.sleep(0.07)
            ...
            """automated escape after 100 seconds """
...
#                    bitVal100 = convertType(100.0, fromType ...unk_multiThread_doornik_testCondition.py= 'f', toType = 'I')
...
#                        self.record = False
            ...
            if (self.record == True):
                ...
                if self.c == len(self.j2List)-1:
                    self.gForearm_body.angle = 0.0
                else:
                    self.gForearm_body.angle = (self.j2List[self.c])*3.141592/180  # in radian   
                    self.c = self.c + 1
                    #print self.j2List[k]  
#            else:
#                k = 0
...
#            label3 = myfont.render("Trial %d/%d, tonicDrive %d, CNgain %.1f" % \
#                                        (self.currTrial, self.REPEAT_NUMBER, \
#                                        self.tonicValue, self.CNGainValue), 1,  THECOLORS["black"])
            self.screen.blit(label1, (10, 10))
            self.screen.blit(label2, (10, 40))
#            self.screen.blit(label3, (10, 55))
...
#                for j in xrange(1):
    ...
    #                     self.currTrial = i
                         """ tonic"""
                         sleep(0.5)
                         self.tonicDrive(self.tonicValue)
                         """ higain"""
                         sleep(1)
                         self.corticalGainControl(self.CNGainValue)
                         
                         print self.CNGainValue, self.tonicValue, i, j
                         sleep(1)
                         #k.type_string('l')
                         if (self.record == False): # play doornik again
                            self.record = True
                         self.c = 1
                         ...
                         self.record = False
                         sleep(1)
                         self.data_bic = []  # empty the queue
                         self.data_tri = []  # empty the queue
                     
            
#                self.tonicValue = self.tonicValue + 250      
#                self.CNGainValue = self.CNGainValue + 1.0   
...
#        threading.Thread(target=self.runDoornik).start()
        ...
        t4 = threading.Thread(target=self.runDoornik)
