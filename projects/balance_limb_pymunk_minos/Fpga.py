...
#        assert self.xem.IsOpen(), "OpalKelly board NOT found!"
        ...
        serX = self.xem.GetDeviceListSerial(self.board)
        print "serial = ",  serX
...
#    def ConfigureXEM(self):
#        bitfile2 = BIT_FILE2
#        assert os.path.exists(bitfile2.encode('utf-8')), ".bit file NOT found!"
#            
##        self.xem = ok.FrontPanel()
##        self.xem.OpenBySerial("")
##        assert self.xem.IsOpen(), "OpalKelly board NOT found!"
#        self.xem2 = ok.FrontPanel()
#        print "count2 = ",  self.xem2.GetDeviceCount()
#        serX = self.xem2.GetDeviceListSerial(1)
#        print "serial2 = ",  serX
#        self.xem2.OpenBySerial(serX)
#        assert self.xem2.IsOpen(), "OpalKelly board NOT found!"
#
#        self.xem2.LoadDefaultPLLConfiguration()
#
#        self.pll = ok.PLL22393()
#        self.pll.SetReference(48)        #base clock frequency
#        self.baseRate = 200 #in MHz
...
#        ## self.pll.SetOutputDivider(1, int(self.baseRate / self.clkRate))       #div4 = 100 mhz
#        ## self.pll.SetOutputEnable(1, True)
#        self.xem2.SetPLL22393Configuration(self.pll)
#        self.xem2.ConfigureFPGA(bitfile2.encode('utf-8'))
#        print(bitfile2.encode('utf-8'))
#        
#        
#    def ReadFPGA(self, getAddr, type):
#
#        """ getAddr = 0x20 -- 0x3F (maximal in OkHost)
#        """
#        self.xem2.UpdateWireOuts()
#        ## Read 18-bit integer from FPGA
#        if type == "int18" :
#            intValLo = self.xem2.GetWireOutValue(getAddr) & 0xffff # length = 16-bit
#            intValHi = self.xem2.GetWireOutValue(getAddr + 0x01) & 0x0003 # length = 2-bit
#            intVal = ((intValHi << 16) + intValLo) & 0xFFFFFFFF
##            intVal = ConvertType(intVal, 'I', 'i')
#            if intVal > 0x1FFFF:
#                intVal = -(0x3FFFF - intVal + 0x1)
...
#        ## Read 32-bit float
#        elif type == "float32" :
#            outValLo = self.xem2.GetWireOutValue(getAddr) & 0xffff # length = 16-bit
#            outValHi = self.xem2.GetWireOutValue(getAddr + 0x01) & 0xffff
#            outVal = ((outValHi << 16) + outValLo) & 0xFFFFFFFF
#            outVal = ConvertType(outVal, 'I', 'f')
#            #print outVal
#        ## Read 32-bit signed integer from FPGA
#        elif type == "int32" :
#            intValLo = self.xem2.GetWireOutValue(getAddr) & 0xffff # length = 16-bit
#            intValHi = self.xem2.GetWireOutValue(getAddr + 0x01) & 0xffff # length = 16-bit
#            intVal = ((intValHi << 16) + intValLo) & 0xFFFFFFFF
#            outVal = ConvertType(intVal, 'I',  'i')  # in mV De-Scaling factor = 128  #????
#
#        ## if getAddr == DATA_OUT_ADDR[0]:
...
#        if evt == BUTTON_RESET:
#            if (buttonValue) :
#                self.xem2.SetWireInValue(0x00, 0x01, 0xff)
#            else :
#                self.xem2.SetWireInValue(0x00, 0x00, 0x01)
#            self.xem.UpdateWireIns()
#        elif evt == BUTTON_RESET_SIM:
#            if (buttonValue) :
#                self.xem2.SetWireInValue(0x00, 0x02, 0xff)
#            else :
#                self.xem2.SetWireInValue(0x00, 0x00, 0x02)
#            self.xem.UpdateWireIns()
#        elif evt == BUTTON_ENABLE_SIM:
#            if (buttonValue) :
#                self.xem2.SetWireInValue(0x00, 0x04, 0xff)
#            else :
#                self.xem2.SetWireInValue(0x00, 0x00, 0x04)
#            self.xem.UpdateWireIns()
#            
#
#
...
#            buf += pack('<f', x) # convert float_x to a byte string, '<' = little endian
#
#        byteSent = self.xem2.WriteToBlockPipeIn(PIPE_IN_ADDR, 4, buf)
#
#        if byteSent == len(buf):
#            print "%d bytes sent via PipeIn!" % byteSent 
#        else:
...
#            buf += pack('<I', x) # convert float_x to a byte string, '<' = little endian
#
#        byteSent = self.xem2WriteToBlockPipeIn(PIPE_IN_ADDR, 4, buf)
#
#        if byteSent == len(buf):
#            print "%d bytes sent via PipeIn!" % byteSent 
#        else:
...
#        bitValLo = bitVal & 0xffff
#        bitValHi = (bitVal >> 16) & 0xffff
#        self.xem2.SetWireInValue(0x01, bitValLo, 0xffff)
#        self.xem2.SetWireInValue(0x02, bitValHi, 0xffff)
#        self.xem2.UpdateWireIns()            
#        self.xem2.ActivateTriggerIn(0x50, trigEvent)   
#
#    def ReadPipe(self, addr, len = 1000):
#        buf = "\x00" * len
#        self.xem2.ReadFromPipeOut(addr, buf)
#        ## 'buf' becomes a string buffer which is used to contain the
#        ## data read from the pipeout. In both the Write and Read
#        ## cases, the length of the buffer passed is the length
#        ## transferred.
