...
#            (filepath, filename) = os.path.split(eachBitFile) 
    ...
    def plotData(self, data):
        ...
        if (data != []):
            ...
            os.chdir('C:\Users\wonjsohn\Dropbox\BBDL_data\sliced_plot_dir\ground1_noForce')
            #savemat(self.fileId + "_" + self.projectName+"_"+timeTag+".mat", {eachName: forplot[:, i] for i, eachName in enumerate(self.allFpgaOutput)})
            savemat(self.fileId + "_" + self.projectName+".mat", {eachName: forplot[:, i] for i, eachName in enumerate(self.allFpgaOutput)})
...
#        print ch.data[0]
    ...
    def tellFpga(self, chanName, newWireIn):
        ...
        if (ctrl.type == 'int32'):
            ...
            # print "bitval before convert, ", newWireIn
            bitVal = convertType(newWireIn, fromType = 'f', toType = 'I')
            # print "bitval is float", bitVal
...
#            pipeInData = gen_sin(F = 1.0, AMP = 0.15,  BIAS = 1.15,  T = 2.0) 
            pipeInData = gen_ramp(T = [0.0, 0.1, 0.3, 0.8, 0.9, 48.0], L = [1.0, 1.0, 1.30, 1.30, 1.0, 1.0], FILT = False)
...
# [flexorLengthThisGamma, newTrialThisGamma, GdArray ,GsArray] = loadMat('C:\Users\wonjsohn\Dropbox\BBDL_data\slicedOutput\slicedTrials_1_0_0_1.mat')
            ...
            #
            # [flexorLengthThisGamma, newTrialThisGamma, GdArray ,GsArray] = loadMat('C:\Users\wonjsohn\Dropbox\BBDL_data\slicedOutput\slicedTrials_2_0_0_2.mat')
            # pipeInData = flexorLengthThisGamma
            ...
            #     if count == 5:
            #         break
            #     #print self.allTriggerOutput['fifo_almost_full'].value

                # print "FIFO FULL?  ", self.nerfModel.IsFIFOFull(0x60)

                # self.nerfModel.UpdateTriggerOuts()
                # if self.nerfModel.IsTriggered(0x60, 0x00) == True:  # fifo not full
                #     [flexorLengthThisGamma, newTrialThisGamma, GdArray ,GsArray] = loadMat('C:\Users\wonjsohn\Dropbox\BBDL_data\slicedOutput\slicedTrials_2_0_0_2.mat')
                #     pipeInData = flexorLengthThisGamma
                #     self.nerfModel.SendPipe(pipeInData)
                #     print "A"
         ...
         # print "current count: ", count
            ...
            # pipeInData = numpy.concatenate([pipeInData,flexorLengthThisGamma])
            # [flexorLengthThisGamma, newTrialThisGamma, GdArray ,GsArray] = loadMat('C:\Users\wonjsohn\Dropbox\BBDL_data\slicedOutput\slicedTrials_3_0_0_3.mat')
            # pipeInData = numpy.concatenate([pipeInData,flexorLengthThisGamma])
            #pipeInData = [pipeInData, flexorLengthThisGamma]
...
#            pipeInData = spike_train(firing_rate = 1000)
    ...
    def fiforeport(self):
        print self.nerfModel.IsFIFOFull(0x60)
        threading.Timer(1.0, self.fiforeport).start()



    def blockram_loop(self, iter):

        print iter


        # [flexorLengthThisGamma, newTrialThisGamma, GdArray ,GsArray] = loadMat('C:\Users\wonjsohn\Dropbox\BBDL_data\slicedOutput\slicedTrials_79_50_200_1.mat')
        [flexorLengthThisGamma, GdArray ,GsArray, cortical, vel] = loadMat('C:\Users\wonjsohn\Dropbox\BBDL_data\sliceBy4Output\slicedTrials_1_0_0_1_1.mat')
        pipeInData = flexorLengthThisGamma
        print GdArray[0][0],  GsArray[0][0], cortical[0][0]
        self.nerfModel.SendPara(bitVal = GdArray[0][0],  trigEvent = 4) # 4 - Gd
        self.nerfModel.SendPara(bitVal = GsArray[0][0], trigEvent =5) # 5 - Gs
        self.nerfModel.SendPara(bitVal = GsArray[0][0], trigEvent =5) # 5 - Gs
        self.nerfModel.SendPipe(pipeInData)


        threading.Timer(28.0, self.blockram_loop, [iter+1]).start() # repeat every 25 seconds.
    ...
    def foo(self, iter):
        ...
        if self.nerfModel.IsFIFOReady(0x60) == 'fifo_almost_empty':  # fifo almost empty. pipe in a chunk.
            [flexorLengthThisGamma, newTrialThisGamma, GdArray ,GsArray] = loadMat('C:\Users\wonjsohn\Dropbox\BBDL_data\slicedOutput\slicedTrials_79_50_200_1.mat')
            pipeInData = flexorLengthThisGamma
            print GdArray[0][0]
            print GsArray[0][0]
            self.nerfModel.SendPara(bitVal = GdArray[0][0],  trigEvent = 4) # 4 - Gd
            self.nerfModel.SendPara(bitVal = GsArray[0][0], trigEvent =5) # 5 - Gs
            self.nerfModel.SendLongPipe(pipeInData)

        elif self.nerfModel.IsFIFOReady(0x60) == 'fifo_almost_full':  # fifo almost empty. pipe in a chunk.
            print "fifo_almost_full"
        else:
            print "do nothing"

        threading.Timer(2.0, self.foo, [iter+1]).start()
